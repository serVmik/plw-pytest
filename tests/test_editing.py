...
from tests.utils import create_default_todos, \
    assert_number_of_todos_in_local_storage, TODO_ITEMS, \
...
def test_should_remove_the_item_if_an_empty_text_string_was_entered(
        page: Page
) -> None:
    todo_items = page.locator('.todo-list li')
    todo_items.nth(1).dblclick()
...
def test_should_cancel_edits_on_escape(page: Page) -> None:
    todo_items = page.locator('.todo-list li')
    todo_items.nth(1).dblclick()
    todo_items.nth(1).locator('.edit').fill('oppa')
